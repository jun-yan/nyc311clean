...
are_valid_values <- function(check_values, valid_values) {
  ...
  check_values <- trimws(check_values)
  valid_values <- trimws(valid_values)
  ...
  valid_set <- unique(valid_values)
  
  # Check if each value exists in the valid set
  check_values %in% valid_set
}
  ...
  observeEvent(input$analyze, {
    ...
    results <- local({
      ...
      invalid_summary <- data.table(
        ZIP_Code = unique(invalid_zips_filtered),
        Count = as.numeric(table(invalid_zips_filtered))
      )
      
      total_records <- nrow(cleaned_data)
      invalid_summary[, Percentage := round(Count / total_records, 6)]
