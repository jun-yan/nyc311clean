...
#########################################################################
# Create a function to replace street types with abbreviations
replace_street_types <- function(address, abbreviations) {
  # Replace multiple spaces with a single space
  address <- gsub("\\s+", " ", address)
  
  # Split the address into individual words
  words <- unlist(strsplit(address, " "))
  
  # Loop through the words and check if they match any abbreviation names
  for (i in seq_along(words)) {
    match <- abbreviations$name == words[i]
    if (any(match)) {
      words[i] <- abbreviations$usps_abbreviation[match]
    }
  }
  
  # Combine the words back into a single string
  replaced_address <- paste(words, collapse = " ")
...
data1File <- file.path( "..","data", "311_Q4_2022_Q1_2023.csv")
writeFilePath <- file.path( "..","data", "311_Q4_2022_Q1_2023_smaller.csv")
...
#functionStart <- as.POSIXct( Sys.time() )

# cleanup street abbreviations
...
xcloned311 <- d311
####################
# Clean up street type names and replace with standard abbreviations
...
xcloned311$cross_street_1 <- sapply(xcloned311$cross_street_1, replace_street_types, USPSabbreviations)
xcloned311$cross_street_2 <- sapply(xcloned311$cross_street_2, replace_street_types, USPSabbreviations)
xcloned311$intersection_street_1 <- sapply(xcloned311$intersection_street_1, replace_street_types, USPSabbreviations)
xcloned311$intersection_street_2 <- sapply(xcloned311$intersection_street_2, replace_street_types, USPSabbreviations)
...
#functionStop <- as.POSIXct( Sys.time() )
#duration <- sprintf( "%.2f", difftime( functionStop, functionStart, units = "secs"))
#cat( "\n\nFunction run-time is: ", duration, "seconds.\n" )
...
####################
cat("\n\n# of matching cross_street_1 & intersection_street_1 is", format(nrow(dupXStreets1), big.mark = ",") ,"representing", round(nrow(dupXStreets1)/nrow(d311)*100,2),"% of total rows.")
