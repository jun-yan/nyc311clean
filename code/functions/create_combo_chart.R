    ...
    ) 
{
  ...
  summary_df <- dataset %>%
    count(agency) %>%
    rename(count = n) %>%
    arrange(desc(count)) %>%  # Sort by count before calculating cumulative_percentage
    mutate(
  ...
  summary_df <- summary_df %>%
    mutate(agency = factor(agency, levels = agency))  # Factor levels based on sorted order
  ...
  # Round the percentage columns to four decimal places
  summary_df <- summary_df %>%
    mutate(
  ...
  # Step 1: Create the initial plot without the hlines
  combo_chart <- ggplot(summary_df) +
    ...
    # Add the y-axis scale with secondary axis for cumulative percentage
    scale_y_continuous(
