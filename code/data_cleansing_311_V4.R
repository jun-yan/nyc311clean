...
USPSabbreviations <-
  read.csv(
    ...
    header = TRUE,
    colClasses = rep("character", ncol(read.csv(data5File)))
  )
...
precinctsNYPD <-
  read.csv(data3File,
    header = TRUE,
    colClasses = rep("character", ncol(read.csv(data3File)))
  )
...
complaintData$percentage <-
  round(prop.table(complaintData$Freq) * 100, 4)
...
colnames(samplecomplaintData) <-
  c("complaint", "count", "percentage", "agency")
...
resolution_action_updated_dateDates <-
  areAllDates(d311$resolution_action_updated_date)

# print out results to the console
cat(
  "\nAre all values in 'closed_date' field dates?",
  closed_dateDates
)
cat("\nAre all values in 'due_date' field dates?", due_dateDates)
cat(
  "\nAre all values in 'created_date' field dates?",
  created_dateDates
)
cat(
  "\nAre all values in 'resolution_action_updated_date' field dates?",
  resolution_action_updated_dateDates
)
...
cat(
  "\nAre all zipcodes in 'incident_zip' five numeric digits?",
  incident_zip5Digits
)

zip_codes5Digits <- areFiveDigits2(d311$zip_codes)
cat(
  "\nAre all zipcodes in 'zip_codes' five numeric digits?",
  zip_codes5Digits
)
...
open_data_channelResults <-
  areInList(d311$open_data_channel_type, data.frame(values = c(
    "UNKNOWN", "MOBILE", "ONLINE", "PHONE", "OTHER"
  )))
cat(
...
vehcile_typeResults <-
  areInList(d311$vehcile_typeResults, data.frame(
    values = c(
...
if (!police_precinctResults$checkIt) {
  ...
  cat(
    "\n\nInvalid values in the 'police_precincts' number",
...
cat(
  ...
  "invalid 'community_board' entries representing",
...
cat(
  ...
  "different 'community_board' entires.\n"
)
...
invalid_zip_df1 <-
  data.frame(
    zip_code = names(invalid_zip_counts1),
...
cat(
  ...
  "invalid 'incident_zip' entries representing",
...
cat(
  ...
  "invalid 'zip_codes' representing",
...
d311$resolution_action_updated_date <-
  strptime(d311$resolution_action_updated_date,
...
cat(
  "\n\n# of SRs 'closed' before they were 'created' (i.e., negative duration):",
  format(nrow(closedBeforeOpened), big.mark = ","),
  "representing",
  round(
    nrow(closedBeforeOpened) / (numRows - numBlankClosedDate) * 100,
    2
  ),
  "% of non-blank data.\n"
)
...
cat(
  "\n\n# of SRs that were 'closed' and 'created' at the exact same time, to the second:",
...
latitude_match <-
  (!is.na(d311$latitude) |
    d311$latitude != "" |
    !is.na(lat)) & round(d311$latitude, 5) == round(lat, 5)
longitude_match <-
  (!is.na(d311$longitude) |
    d311$latitude != "" |
    !is.na(long)) & round(d311$longitude, 5) == round(long, 5)

# Get the rows where latitude or longitude does not match
mismatched_rows <- d311[!latitude_match | !longitude_match, ]
mismatched_rows <-
  mismatched_rows[complete.cases(mismatched_rows[, c("latitude", "longitude", "location")]), ]
...
if (nrow(mismatched_rows) > 0) {
  ...
  cat(
    ...
    "non-matches between 'latitude' & 'longitude' and 'location'.\n"
  )
  print(head(mismatched_rows, 5))
} else {
  cat("\nAll values of the 'latitude' & 'longitude' fields match those in the 'location' field.")
}
...
cat(
  "\n\n# of non-matches between 'borough' and 'borough_boundaries' are",
...
cat(
  "\nNon-matches between the 'borough' and 'taxi_company_borough' fields (excluding blanks) number",
  format(nrow(nonMatchingtaxi_company_borough), big.mark = ","),
  "representing",
  round(
    nrow(nonMatchingtaxi_company_borough) / (numRows - numBlanktaxi_company_borough) *
      100,
    2
  ),
  "% of non-blank data\n"
...
for (col in address_fields) {
  xcloned311[[col]] <-
    gsub(pattern, replacement, xcloned311[[col]], perl = TRUE)
}
...
for (field in address_fields) {
  xcloned311[affected_rows1, field] <-
    ifelse(grepl("UNNAMED|NONAME", xcloned311[affected_rows1, field]), "", xcloned311[affected_rows1, field])
  xcloned311[affected_rows1, field] <-
    gsub("\\?+", " ", xcloned311[affected_rows1, field])
}
...
for (field in address_fields) {
  xcloned311[affected_rows1, field] <-
    ifelse(grepl(" E$", xcloned311[affected_rows1, field]),
      paste0("E ", gsub(" E$", "", xcloned311[affected_rows1, field])),
      xcloned311[affected_rows1, field]
    )

  xcloned311[affected_rows3, field] <-
    ifelse(grepl(" W$", xcloned311[affected_rows3, field]),
      paste0("W ", gsub(" W$", "", xcloned311[affected_rows3, field])),
      xcloned311[affected_rows3, field]
    )

  xcloned311[affected_rows3, field] <-
    ifelse(grepl(" N$", xcloned311[affected_rows3, field]),
      paste0("N ", gsub(" N$", "", xcloned311[affected_rows3, field])),
      xcloned311[affected_rows3, field]
    )

  xcloned311[affected_rows3, field] <-
    ifelse(grepl(" S$", xcloned311[affected_rows3, field]),
...
for (i in seq_along(patterns)) {
  ...
  for (col in address_fields) {
    ...
    xcloned311[, col] <-
      ifelse(logical_vector,
        gsub(pattern, replacement, xcloned311[, col]),
        xcloned311[, col]
      )
  }
}
...
for (col in address_fields) {
  ...
  xcloned311[, col] <-
    ifelse(logical_vector,
      gsub("^AVE ", "AVENUE ", xcloned311[, col]),
      xcloned311[, col]
    )
  ...
  xcloned311[, col] <-
    ifelse(logical_vector,
      gsub("^E END AVE$", "EAST END AVE", xcloned311[, col]),
      xcloned311[, col]
    )
  ...
  xcloned311[, col] <-
    ifelse(logical_vector,
      gsub("^W END AVE$", "WEST END AVE", xcloned311[, col]),
      xcloned311[, col]
    )

  # Update addresses
  xcloned311[, col] <-
    gsub("^(\\d+) (\\w+) (\\w+)$", "\\3 \\1 \\2", xcloned311[, col])
}
...
####################
cat(
  "\n\n# of matching 'cross_street_1' & 'intersection_street_1' is",
...
cat(
  "\n# of non-matching 'cross_street_1' & 'intersection_1' is",
...
print(head(nondupXStreets1[nondupXStreets1$cross_street_1 != "" &
  nondupXStreets1$intersection_street_1 != "", ], 15))
...
cat(
  "\n\n# of ocurrences where 'cross_street_1' is blank, but 'intersection_street_1' is not blank is",
...
cat(
  "\n# of ocurrences where 'intersection_street_1' is blank, but 'cross_street_1' is not blank",
...
cat(
  "\n\n# of matching 'cross_street_2' & 'intersection_2' is",
...
cat(
  "\n\n# of non-matching 'cross_street_2' & 'intersection_street_2' is",
...
print(head(nondupXStreets2[nondupXStreets2$cross_street_2 != "" &
  nondupXStreets2$intersection_street_2 != "", ], 15))
...
cat(
  "\n\n# of ocurrences where 'cross_street_2' is blank, but 'intersection_street_2' is not blank is",
...
cat(
  "\n\n# of ocurrences where 'cross_street_2' is not blank, but 'intersection_street_2' is blank ",
...
if (nrow(match_1 > 0)) {
  cat(
    "\n\nThere are",
    nrow(match_1),
    "near-matches between 'cross_street_1' & 'intersection_street_1'. Here is a sample:\n"
  )
...
if (nrow(match_2 > 0)) {
  cat(
    "\nThere are",
    nrow(match_2),
    "near-matches between 'cross_street_2' & 'intersection_street_2'. Here is a sample:\n"
  )
...
duration <-
  sprintf("%.2f", difftime(programStop, programStart, units = "secs"))
