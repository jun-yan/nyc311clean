...
create_bar_chart <- function(
  ...
  cat("\n")
  cat(
    paste("Average ", print_out_title, ":", sep = ""), format(y_mean_value, big.mark = ","), "  ",
...
########################################################################
# Aggregate by calendar month with 3-letter abbreviations
...
SR_calendar_month <- ggplot(calendar_month_df, aes(x = Month, y = count)) +
  ...
  theme(
    ...
    plot.subtitle = element_text(size = 9),
    panel.background = element_rect(fill = "gray95", color = "gray95"),
    axis.text.x = element_text(angle = 60, vjust = 0.65, hjust = 0.5, face = "bold"),
    axis.text.y = element_text(face = "bold")
  ) +
  ggtitle("SRs by Calendar Month",
  ...
  annotate("text",
    x = earliest_month, y = mean_count,
    label = paste0("Average: ", format(round(mean_count, 0), big.mark = ",")),
    size = 4, hjust = -0.5, vjust = -0.75
  ) +
  annotate("text",
    x = max_calendar_month_name, y = max_calendar_count,
    label = paste0("Max: ", format(max_calendar_count, big.mark = ",")),
    size = 4, color = "black", vjust = -0.6, hjust = 0.5
  ) +
  labs(x = NULL, y = NULL)

# Print the bar chart
suppressMessages(print(SR_calendar_month))
chart_path <- file.path(chart_directory_path, paste0(file_name_prefix, "-trend_by_calendar_month.pdf"))
...
SR_day_of_the_year <- ggplot(day_counts_df, aes(x = day_of_year, y = count)) +
  ...
  ggtitle("SRs by Day-of-the-Year",
    subtitle = paste(
  ...
  geom_hline(yintercept = mean_count, linetype = "twodash", color = "black", linewidth = 0.6) +
  annotate("text",
    x = earliest_day_of_year, y = mean_count,
    label = paste0("Average: ", format(round(mean_count, 0), big.mark = ",")),
    size = 4, hjust = -0.5, vjust = -0.75
  ) +
  annotate("text",
    x = max_day_of_the_year$day_of_year, y = max_day_of_the_year$count,
    label = paste0("Max: ", format(max_day_of_the_year$count, big.mark = ",")),
    size = 4, color = "black", vjust = -0.6, hjust = 0.3
  ) +
  scale_x_discrete(
    expand = c(0.01, 0),
    breaks = day_counts$day_of_year[seq(1, nrow(day_counts), by = 19)]
  ) +
  labs(y = NULL, x = NULL)

# Print the bar chart
suppressMessages(print(SR_day_of_the_year))
chart_path <- file.path(chart_directory_path, paste0(file_name_prefix, "-trend_by_day_of_year.pdf"))
...
SR_day_of_the_week <- ggplot(day_of_week_df, aes(x = day_of_week, y = count)) +
  ...
  ggtitle("SRs by Day-of-the-Week",
    subtitle = paste(
      "(", earliest_title, "--", latest_title, ")",
      "total=", format(total_count, big.mark = ",")
    )
  ...
  geom_hline(yintercept = mean_count, linetype = "twodash", color = "black", linewidth = 0.6) +
  annotate("text",
    x = earliest_day_of_week, y = mean_count,
    label = paste0("Average: ", format(round(mean_count, 0), big.mark = ",")),
    size = 4, hjust = -0.5, vjust = -0.75
  ) +
  annotate("text",
    x = max_day_of_the_week$day_of_week, y = max_count,
    label = paste0("Max: ", format(max_count, big.mark = ","), sep = ""),
    size = 3.7, color = "black", vjust = -0.7, hjust = 0.5
  ) +
  labs(x = NULL, y = NULL)

# Print the bar chart
suppressMessages(print(SR_day_of_the_week))
chart_path <- file.path(chart_directory_path, paste0(file_name_prefix, "-trend_by_day_of_week.pdf"))
