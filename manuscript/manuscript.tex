...
\label{sec:blanks}
Understanding the absence of data by field greatly aids when 
conducting analysis. For example, determining if SRs were closed 
before or after their \texttt{due\_date}, you would be challenged as 99.6\% of the
\texttt{due\_date} fields are blank, and are only used by one Agency, DSNY. Blank 
(or N/A values) they appear to divide into three groups:
...
to a domain of allowable values, including: 
...
\begin{quote}
\textit{Note: The authors attempted extensive standardization of street names  
in an attempt to match what were obvious identical streets, deemed unequal 
due only to spelling and naming conventions. Those efforts were intended to match, for example, 
"KANE PL" and "KANE PLACE" as well as "FLATLANDS 7 ST" and "FLATLANDS 7 STREET". However, these 
efforts were  abandoned after they increased street name matches by less than 1\%.)}
\end{quote}
...
data. Such errors could potentially mislead analytic efforts. Examples include:
