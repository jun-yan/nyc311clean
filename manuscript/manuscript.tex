...
While one can likely infer the meaning of these \emph{computed} 
...
as experimental and \emph{not for public use}. An update 
...
and ``\texttt{<NA>}''. This variety of representations complicates programming
...
be \emph{created} at 1:59AM and then be \emph{closed} 30 minutes later 
...
Figure~\ref{fig:blank_fields} is a graphic depiction of the percentage of
...
mirrors the \emph{overall} breakdown of SRs by Agency, potentially 
...
\begin{itemize}[left=1.5em]
\item 12,450 SRs with a \texttt{closed\_date} before
  \texttt{created\_date} creating a nonsensical ``negative duration''. 
\item Eight SRs with \texttt{created\_date(s)} or \texttt{closed\_date(s)} in 
  the distant past, specifically  \texttt{1900-01-01}. Despite few of 
...
\texttt{closed\_date}(s) to measure the \emph{duration} of SRs, 
...
of \texttt{1900-01-01} as the \texttt{closed\_date}. This results 
...
Excluding the extreme negative values (any that are lower than $-$730 days), 
...
of these negative durations (some lower than $-$300 days) is troubling 
...
infeasible dates such as \texttt{1900-01-01}. The chart 
...
\texttt{KANE PL} and \texttt{KANE PLACE} or \texttt{AVE P} and
\texttt{AVENUE P}. However, this standardization effort was
...
``\texttt{NOISE - STREET/SIDEWALK}'' could be represented by a 
numeric code. For this dataset this code occurs 
...
that the \texttt{status} field takes on values of \texttt{assigned, canceled, 
closed}, or \texttt{pending}. However, we observed additional values 
such as \texttt{in progress, started}, and \texttt{unspecified}. Additionally, 
no SRs were found with a status of \texttt{canceled}. Similarly, 
the Data Dictionary for the \texttt{address\_type} field lists as acceptable
values \texttt{address, blockface, intersection, latlong}, and 
\texttt{placename}. However, we found additional values such 
as \texttt{bbl} and \texttt{unrecognized}. Additionally, no 
\texttt{latlong} values were observed. Other fields, such 
...
standards, such as the percentage of invalid zip codes. Such tools could
