...
% Force a blank line and remove indentation
\vspace{\baselineskip}
\noindent
...
\vspace{\baselineskip}
\noindent
The accomanying R program also produces a CSV file showing Field usage by Agency. 
Table~\ref{tab:field-usage-by-agency} contains a sample of Field usage by select Agencies.

\begin{table}[tbp]
\centering
\caption{Sample Field Usage by select Agencies}
...
\begin{quote}
\textit{Note: The authors attempted standardization of street names  
...
\textit{KANE PL} and \textit{KANE PLACE} as well as \textit{AVE P} and \textit{AVENUE P}. However, these 
efforts were  abandoned after they resulted in an increase in street name matches of less than 1\%.)}
\end{quote}
...
due to their sparse data population. This caveat applies to the fields 
...
large (6.4 million row) dataset.; certainly a non-trivia amount of data.}
\end{quote}
